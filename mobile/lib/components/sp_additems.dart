...
class _SPAddItemsState extends State<SPAddItems> {
  ...
  String? validatePrice(String? value) {
    ...
    if (!RegExp(r'^Rs:\d{4}/=$').hasMatch(value)) {
      return 'Price should be in the format Rs:####/=(e.g., Rs:1234/=)';
    }
    return null;
  }
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 35.0),
           buildEditableField(itemNameController, 'Item Name', validator: (value) {
  if (value == null || value.isEmpty) {
    return 'Please enter an item name';
  }
  return null;
}),
buildEditableField(descriptionController, 'Description', validator: (value) {
  if (value == null || value.isEmpty) {
  ...
  return null;
              ...
              buildEditableField(priceController, 'Price',
                  keyboardType: TextInputType.text, validator: validatePrice),
              buildEditableField(quantityController, 'Quantity',
                  keyboardType: TextInputType.text, validator: validateQuantity),
              SizedBox(height: 20.0),
              Container(
                alignment: Alignment.center,
                child: ElevatedButton(
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      // Validation successful, proceed to add the item
                      final itemName = itemNameController.text;
                      final description = descriptionController.text;
                      final price = double.tryParse(priceController.text) ?? 0.0;
                      final quantity = int.tryParse(quantityController.text) ?? 0;
